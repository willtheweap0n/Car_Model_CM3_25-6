...
FIXED_GEAR_RATIOS_DICT = {
    i + 1: ratio for i, ratio in enumerate(FIXED_GEAR_RATIOS_LIST)
}
...
OUTPUT_FILENAME = "final_route_data.csv"
# Note: Ideally, store API keys in environment variables
...
SPEED_LIMIT_DEFAULT_MPH = 70.0

# Rules for the END of the route (Distance REMAINING -> Speed Limit)
# These are processed FIRST (highest priority).
MANUAL_SPEED_LIMITS_END = [
    (2.0, 20.0),  # Final 2k
    (3.0, 30.0),  # 3k to 2k from end
    (8.0, 40.0)   # 8k to 3k from end
]

# Rules for the START of the route (Distance from Start -> Speed Limit)
MANUAL_SPEED_LIMITS_START = [
    (10.0, 20.0),  # 0-10km
    (12.0, 30.0),  # 10-12km
    (13.0, 40.0),  # 12-13km
    (14.0, 50.0)   # 13-14km
...
def calculate_point_distances(df_route):
    ...
    lats = df_route['lat'].values
    lons = df_route['lon'].values
...
# --- 3. CORE ENGINE MODEL (INTERPOLATION) ---
get_fuel_flow = None
get_max_torque = None
get_min_torque = None
...
def setup_engine_model():
    ...
    # Formatted for PEP 8 readability (broken into chunks)
    ...
    wot_speed_rads = np.array(
        [0.0, 95.0, 157.2, 178.0, 209.5, 235.7, 256.4, 282.7, 314.1, 356.1, 429.4, 628.3, 680.7]
    )
    wot_torque_nm = np.array(
        [0.0, 134.6, 164.9, 167.8, 187.9, 188.3, 182.3, 188.2, 201.4, 197.4, 200.1, 184.0, 0.0]
    )
    ...
    _get_fuel_flow = RectBivariateSpline(
        torque_nm_axis, speed_rads_axis, fuel_gps_map, kx=1, ky=1
    )
    _get_max_torque = interp1d(
        wot_speed_rads * rads_to_rpm, wot_torque_nm,
        kind='linear', bounds_error=False, fill_value=0.0
    )
    _get_min_torque = interp1d(
        ct_speed_rads * rads_to_rpm, ct_torque_nm,
        kind='linear', bounds_error=False, fill_value=-16.0
    )
...
def calculate_resistance_forces(speed_mps, slope_percent):
    """Calculate total resistance forces (Drag, Rolling, Gravity)."""
...
# --- 5. ROUTE PRE-PROCESSING ---
PROCESSED_ROUTE = []


def preprocess_route():
    """Loads route CSV and processes it into micro-segments."""
    ...
    try:
        ...
        print(f"❌ ERROR: Route file not found: '{ROUTE_FILENAME}'")
        print("Please run this script's `create_route_file()` function first.")
        return False
    except Exception as e:
        print(f"❌ ERROR: Could not read route file: {e}")
        return False
    ...
    df_route['slope_pct'] = (
        df_route['elev_change_m'] / df_route['dist_to_next_safe']
    ) * 100
    ...
    print(f"Route pre-processed. {len(PROCESSED_ROUTE)} micro-segments. "
          f"Total distance: {total_dist_km:.1f} km")
    return True


# --- 6. HYBRID SIMULATION "MINI-FUNCTIONS" ---
...
def simulate_cruise(speed_mps, slope_pct, distance_m, final_drive_ratio):
    ...
    if cache_key == _cruise_cache_key:
        ...
        for gear, ratio in FIXED_GEAR_RATIOS_DICT.items():
            ...
            if rpm < MIN_ENGINE_RPM or rpm > MAX_ENGINE_RPM:
                continue

            engine_tq_req = (wheel_torque_req) / (ratio * final_drive_ratio)

            if (engine_tq_req < get_min_torque(rpm) or
                    engine_tq_req > get_max_torque(rpm)):
                continue

            current_fuel_flow = get_fuel_flow(rpm, engine_tq_req)
            if engine_tq_req < 0:
                current_fuel_flow = 0.0  # DFCO (Deceleration Fuel Cut Off)
...
def _accel_ode_model(t, y, slope_pct, final_drive_ratio):
    ...
    rpm_at_max_power = get_engine_rpm_from_speed(
        speed_mps, best_power_gear_ratio, final_drive_ratio
    )
    engine_torque_req = get_max_torque(rpm_at_max_power) * 0.80
    fuel_flow_gps = get_fuel_flow(rpm_at_max_power, engine_torque_req)

    f_tractive = (
        (engine_torque_req * best_power_gear_ratio * final_drive_ratio) /
        TIRE_RADIUS_M
    )
    f_resistance = calculate_resistance_forces(speed_mps, slope_pct)
...
def simulate_acceleration(start_speed_mps, target_speed_mps,
                          slope_pct, final_drive_ratio):
...
# --- 7. HYBRID SIMULATION ENGINE ---

def run_segment_simulation(final_drive_ratio):
    """Loops through route segments to calculate total fuel/time."""
    ...
    for (seg_dist_m, seg_slope_pct, seg_speed_limit_mps) in PROCESSED_ROUTE:
        ...
        if target_speed_mps > current_speed_mps:
            (fuel_g, time_s, dist_m, final_speed) = simulate_acceleration(
                ...
                seg_slope_pct, final_drive_ratio
            )
            ...
            if remaining_distance_m > 0:
                (fuel_g_cruise, time_s_cruise) = simulate_cruise(
                    current_speed_mps, seg_slope_pct,
                    remaining_distance_m, final_drive_ratio
                )
...
# --- 8. OPTIMIZATION PROBLEM SETUP ---

def objective_function(final_drive_ratio):
    """The 'Worker' function called by the optimizer."""
    ...
    if fuel_liters == 0.0:
        print(f"  FD: {final_drive_ratio:.3f} | Simulation FAILED (Cost: 0.0 L) "
              f"| Time: {end_time - start_time:.2f}s")
        return float('inf')

    print(f"  FD: {final_drive_ratio:.3f} | Cost: {cost:.5f} L "
          f"| Time: {end_time - start_time:.2f}s")
    return cost


# --- 9. ROUTE MAPPER FUNCTIONS ---

def get_route_from_ors(start_coords, end_coords, api_key):
    """Get route data from OpenRouteService (ORS) API."""
    print(f"🗺️  Getting route from {start_coords} to {end_coords} via ORS...")
    ...
    try:
        ...
        if 'features' not in data or len(data['features']) == 0:
            print("❌ No route found between these coordinates")
            return None
        ...
        if len(coords[0]) == 3:
            ...
            print(f"   ✅ Route found: {len(df)} points with REAL elevation")
        else:
            df = pd.DataFrame(coords, columns=["lon", "lat"])
            df["elev_m"] = 0.0  # Fallback
            print(f"   ✅ Route found: {len(df)} points (no elevation data)")

        return df

    except Exception as e:
        print(f"❌ ORS API call failed: {e}")
        return None


def apply_manual_speed_limits(df_route):
    """Applies user-defined speed limit logic using configured constants."""
    print("   - Applying manual speed limit logic...")

    # Calculate the cumulative distance in km for each point
    df_route['cumulative_distance_km'] = (
        df_route['distance_to_next_m'].cumsum() / 1000.0
    )
    ...
    # 1. END OF ROUTE RULES (Highest Priority)
    for dist_from_end, speed_mph in MANUAL_SPEED_LIMITS_END:
        cut_off_point = total_route_distance_km - dist_from_end
        conditions.append(df_route['cumulative_distance_km'] > cut_off_point)
        choices.append(speed_mph)

    # 2. START OF ROUTE RULES (Lower Priority)
    for dist_from_start, speed_mph in MANUAL_SPEED_LIMITS_START:
        conditions.append(df_route['cumulative_distance_km'] <= dist_from_start)
        choices.append(speed_mph)

    # The 'default' is the highway speed
    df_route['speed_limit_mph'] = np.select(
        conditions, choices, default=SPEED_LIMIT_DEFAULT_MPH
    )
...
def print_route_summary(df_route):
    ...
    print(f"Total Distance: {total_distance_m / 1000:.2f} km "
          f"({total_distance_miles:.2f} miles)")
    print(f"Elevation Range: {df_route['elev_m'].min():.1f}m to "
          f"{df_route['elev_m'].max():.1f}m")
    ...
    for speed, count in speed_stats.items():
        ...
        print(f"  {speed} mph ({kmh_equivalent} km/h): "
              f"{count} points ({percentage:.1f}%)")

    avg_speed_limit_mph = df_route['speed_limit_mph'].mean()
    avg_speed_limit_kmh = mph_to_kmh(avg_speed_limit_mph)
    print(f"\nAverage Speed Limit: {avg_speed_limit_mph:.1f} mph "
          f"({avg_speed_limit_kmh:.1f} km/h)")


def create_route_file():
    """Main function to fetch and process the route."""
    print("=" * 50)
    print("🚗 1. RUNNING ROUTE MAPPER")
    print("=" * 50)

    print(f"\n📍 Hard-coded route: Edinburgh {START_COORDS} → Glasgow {END_COORDS}")
    ...
    if df_route is not None:
        ...
        print(f"\n✅ Successfully saved final route data to '{OUTPUT_FILENAME}'")

        # 5. Print Summary
        print_route_summary(df_processed)

        # Show sample of the enhanced data
        print(f"\n📋 Sample of final data (first 5 points):")
        print(df_processed.head().to_string(index=False))
        return True

    else:
        print("❌ Route generation failed. Exiting.")
        return False


# --- 10. TOP SPEED & GRADIENT FINDER (ROOT FINDING) ---

def _root_find_function(speed_mps, gear, slope_percent,
                        final_drive_ratio, get_max_torque_at_rpm):
    """
    The f(x) for Newton-Raphson.
    Returns Net Force (Tractive - Resistance). Target is 0.
    """
    if speed_mps < 0:
    ...
    current_rpm = get_engine_rpm_from_speed(
        speed_mps, gear_ratio, final_drive_ratio
    )
...
def _root_find_derivative_numeric(f, speed_mps, args, h=0.01):
    """
    Calculates f'(x) numerically (central difference).
    f'(x) ≈ ( f(x+h) - f(x-h) ) / 2h
    """
...
def newton_raphson_solver(f, f_prime, x0, args=(), tol=0.01, max_iter=50):
    """Manual implementation of the Newton-Raphson algorithm."""
    x_n = x0
...
def find_top_speed(final_drive_ratio, gear, slope_percent, get_max_torque_func):
    """Wrapper for the Newton-Raphson root finder for Speed."""
...
# --- 11. PROBLEM 2: FIND MAX SLOPE (GRADEABILITY) ---

def _find_max_slope_f(slope_percent, speed_mps, gear,
                      final_drive_ratio, get_max_torque_at_rpm):
    """This is the f(x) for the Max Slope problem. We solve for slope."""

    # Calculate Tractive Force at WOT (this is now a constant)
    gear_ratio = FIXED_GEAR_RATIOS_DICT[gear]
    current_rpm = get_engine_rpm_from_speed(
        speed_mps, gear_ratio, final_drive_ratio
    )
    ...
    # Calculate Resistance Force (this is now the variable)
    f_resistance = calculate_resistance_forces(speed_mps, slope_percent)

    return f_tractive - f_resistance


def find_max_slope(final_drive_ratio, gear, speed_kmh):
    """Wrapper for the Newton-Raphson solver for max slope."""
    initial_guess_slope = 5.0  # Guess 5% grade
    speed_mps = speed_kmh / 3.6
    args_tuple = (speed_mps, gear, final_drive_ratio, get_max_torque)

    max_slope_pct = newton_raphson_solver(
        _find_max_slope_f,
        _root_find_derivative_numeric,
        initial_guess_slope,
        args=args_tuple
...
if __name__ == "__main__":
    try:
        ...
        if not create_route_file():  # This now calls the MANUAL mapper
            raise SystemExit(
                "\n🛑 Halting execution. Route file could not be generated."
            )
        ...
        if not preprocess_route():
            raise SystemExit(
                "\n🛑 Halting execution. Route file could not be processed."
            )
        ...
        if baseline_fuel == float('inf'):
            ...
            if result.success:
                ...
                print(f"Baseline Ratio: {BASE_FINAL_DRIVE_RATIO:.3f} | "
                      f"Fuel: {baseline_fuel:.5f} L")
                print(f"Optimal Ratio:  {optimal_ratio:.3f} | "
                      f"Fuel: {final_fuel_cost:.5f} L")
                
                fuel_saved = baseline_fuel - final_fuel_cost
                percent_saved = (fuel_saved / baseline_fuel) * 100
                print(f"\nFuel Saved: {fuel_saved:.5f} L "
                      f"({percent_saved:.2f}% improvement)")
            else:
        ...
        top_speed_base = find_top_speed(
            BASE_FINAL_DRIVE_RATIO, 6, 0.0, get_max_torque
        )
        ...
        # --- TASK 6: RUN MAX SLOPE (GRADEABILITY) ANALYSIS ---
        print("\n--- TASK 6: RUNNING MAX SLOPE ANALYSIS (NEWTON-RAPHSON) ---")
        ...
        max_slope_base = find_max_slope(
            BASELINE_RATIO, GEAR_TO_TEST, SPEED_TO_TEST_MPS * 3.6
        )
        print(f"  > Max Climbable Grade at {SPEED_TO_TEST} mph "
              f"(Gear {GEAR_TO_TEST}): {max_slope_base:.2f}%")

        print(f"\nTesting 'OPTIMAL' Final Drive: {optimal_ratio:.3f}")
        max_slope_opt = find_max_slope(
            optimal_ratio, GEAR_TO_TEST, SPEED_TO_TEST_MPS * 3.6
        )
        print(f"  > Max Climbable Grade at {SPEED_TO_TEST} mph "
              f"(Gear {GEAR_TO_TEST}): {max_slope_opt:.2f}%")

        
    except ImportError:
        print("\n--- ERROR ---")
        print("This script requires 'numpy', 'matplotlib', 'scipy', 'pandas', "
              "and 'requests'.")
