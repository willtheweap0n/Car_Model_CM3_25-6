...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 14 10:10:21 2025
...
def simulate_cruise(speed_mps, slope_pct, distance_m, final_drive_ratio):
    ...
    if cache_key == _cruise_cache_key:
        ...
        for gear, ratio in FIXED_GEAR_RATIOS_DICT.items():
            ...
            if engine_tq_req < 0: current_fuel_flow = 0.0
...
def get_route_from_ors(start_coords, end_coords, api_key):
    print(f"Getting route from {start_coords} to {end_coords} via ORS...")
...
def apply_manual_speed_limits(df_route):
    ...
    end_minus_3k = total_route_distance_km - 3.0
    end_minus_2k = total_route_distance_km - 2.0
    
    conditions = [
        (df_route['cumulative_distance_km'] > end_minus_2k),
        (df_route['cumulative_distance_km'] > end_minus_3k),
        (df_route['cumulative_distance_km'] > end_minus_8k),
        (df_route['cumulative_distance_km'] <= k_10),
        (df_route['cumulative_distance_km'] <= k_12),
        (df_route['cumulative_distance_km'] <= k_13),
        (df_route['cumulative_distance_km'] <= k_14),
    ]
...
def create_route_file():
    ...
    print(f"\nHard coded route: Edinburgh {START_COORDS} to Glasgow {END_COORDS}")
