...
def calculate_point_distances(df_route):
    ...
    for i in range(len(df_route)-1):
        distances[i] = haversine_distance(lats[i], lons[i], lats[i+1], lons[i+1])
...
# --- 6. PROBLEM 2: FIND MAX SLOPE (GRADEABILITY) ---

def _find_max_slope_f(slope_percent, speed_mps, gear, final_drive_ratio, get_max_torque_at_rpm):
    """ This is the f(x) for the Max Slope problem. We solve for slope. """
    
    # Calculate Tractive Force at WOT (this is now a constant)
    gear_ratio = FIXED_GEAR_RATIOS_DICT[gear]
    current_rpm = get_engine_rpm_from_speed(speed_mps, gear_ratio, final_drive_ratio)
    max_engine_torque = get_max_torque_at_rpm(current_rpm)
    total_reduction = gear_ratio * final_drive_ratio
    f_tractive = (max_engine_torque * total_reduction) / TIRE_RADIUS_M
